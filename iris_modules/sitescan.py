...
def print_result(data, email, websites):
    ...
    for results in data:
        if '@' in email:
            ...
            if results["exists"] == True: found += 1; print(f"{Fore.GREEN}•{Fore.RESET} https://{domain}{Style.RESET_ALL}")
        else:
            domain = results['domain'].replace('username', email)
            if results["exists"] == True: found += 1; print(f"{Fore.GREEN}•{Fore.RESET} https://{domain}{Style.RESET_ALL}")

    print(f'{Fore.MAGENTA}•{Fore.RESET} Found {found} Results For {email}')
    print()
    
async def main(email):
    print(f'{Fore.YELLOW}•{Fore.RESET} Searching for {email}...')
